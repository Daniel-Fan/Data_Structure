...
{ 6,StackStr - WarnStr - 1,LinkerSubM,EnvironStr - LinkerStr - 1,LinkerStr },
{ 8,EditAutoStr - ConfigStr - 1,EnvironSubM,DirectStr - EnvironStr - 1,EnvironStr },
{ 6,OutputStr - LibraryStr - 1,DirectSubM,ArgumentStr - DirectStr - 1,DirectStr },
...
void ShowMenu()
{
	...
	for (His[Layer].MenuIndex = 0; His[Layer].MenuIndex<His[Layer].MenuItemTotal; His[Layer].MenuIndex++)
	{
		CalculatePosition();
		gotoxy(coordcurrent.X, coordcurrent.Y);
		textattr(CurColor);
...
//////计算最大值，用于计算子菜单的宽度
int countmax(int data[],int count)
{
	int maxx = 0;
	int i;
	for(i = 0; i<count; i++)
...
void Upcursor()
{
	...
	changemenu();
	CurColor = SelectAttr;
	if (His[Layer].MenuIndex == 0)
	{
		His[Layer].MenuIndex = His[Layer].MenuItemTotal - 1;
	}
	else {
		His[Layer].MenuIndex = His[Layer].MenuIndex - 1;
	}
	changemenu();
}
...
void Rightcursor()
{
	if (Layer == 1)     //在第一级菜单打开后，左键会使主菜单左移一位
	{
		Esckey();
		CurColor = NoSelectAttr;
		changemenu();
		CurColor = SelectAttr;
		if (His[Layer].MenuIndex == His[Layer].MenuItemTotal - 1)  //在最后一个项时，重新回到第一项
		{
			His[Layer].MenuIndex = 0;
		}
		else {
	...
	else
	{
		...
		changemenu();
		CurColor = SelectAttr;
		if (His[Layer].MenuIndex == His[Layer].MenuItemTotal - 1)
		{
			His[Layer].MenuIndex = 0;
		}
		else {
...
void Downcursor()
{
	...
	else
	{
		...
		changemenu();
		CurColor = SelectAttr;
		if (His[Layer].MenuIndex == His[Layer].MenuItemTotal - 1)  //在第一个项时，重新回到最后一项
		{
			His[Layer].MenuIndex = 0;
		}
		else {
